...
namespace ldmx {
    namespace test {
        ...
        void matchesAll(const ldmx::DoubleTableCondition& a, const ldmx::DoubleTableCondition& b) {
            matchesMeta(a,b);
            for (unsigned int i=0; i<a.getRowCount(); i++) {
                ...
                for (unsigned int ic=0; ic<a.getColumnCount(); ic++) {
                    //	  std::cout << i << ',' << ic << "=>" << ar.second[ic] << " " << br.second[ic] << std::endl;
                    REQUIRE( fabs(ar.second[ic]-br.second[ic])/std::max(1e-5,(ar.second[ic]+br.second[ic])/2) < 1e-5 ); // limited precision of course
                }
            }
        }
